...
class AgentOrchestrationService(
    ...
    companion object {
        private const val MAX_TOOL_CALL_ROUNDS = 50
    }
...
//                logger.debug { "[$requestId] [10/12] 用户输入文本长度: ${userText.length} chars" }
                ...
                val agentPrompt = prompt(
                    id = "agent-chat",
                    params = LLMParams(
                        temperature = ctx.effectiveLLMConfig.temperature.toDouble(),
...
//                logger.debug { "[$requestId] [11/12] Agent配置完成: maxIterations=$MAX_TOOL_CALL_ROUNDS" }
                ...
                // 定义策略图 - 使用标准 nodeLLMRequest 支持工具调用条件边
                // 流式输出通过 handleEvents.onLLMStreamingFrameReceived 发送 SSE 事件
...
//                logger.info { "[$requestId] [12/12] 启动AIAgent执行流程..." }
                ...
                val agent = AIAgent(
                    ...
                    handleEvents {
                        ...
                        onLLMStreamingFrameReceived { ctx ->
                            when (val sf = ctx.streamFrame) {
                                ...
                                is StreamFrame.ToolCall -> {
                                    logger.debug { "[$requestId] LLM工具调用信号: toolCallId=${sf.id}, tool=${sf.name}" }
                                }
                        ...
                        onLLMStreamingFailed { ctx ->
                            logger.error { "[$requestId] LLM流式输出发生错误: ${ctx.error}" }
                        }
                        onLLMStreamingCompleted {
                            logger.debug { "[$requestId] LLM流式输出完成" }
                        }
                        ...
                        onToolCallCompleted { tcCtx ->
                            ...
                            logger.debug { "[$requestId] 工具调用完成 [#$toolCallCount]: tool=${tcCtx.toolName}, resultLength=${resultText.length}, resultPreview=$resultPreview" }
                            send(AgentStreamEvent.ToolCallResult(tcCtx.toolName, resultText))
                        }
                        onToolCallFailed { tcCtx ->
                            logger.error { "[$requestId] 工具调用失败 [#$toolCallCount]: tool=${tcCtx.toolName}, error=${tcCtx.error?.message}" }
                            // 发送错误结果，让 LLM 可以继续处理
                            send(
                                AgentStreamEvent.ToolCallResult(
                                    tcCtx.toolName,
                                    "Error: ${tcCtx.error?.message ?: "Unknown error"}"
                                )
                            )
                        }
                        onAgentCompleted { acCtx ->
                            logger.debug { "[$requestId] Agent执行完成: totalToolCalls=$toolCallCount, resultLength=${acCtx.result.toString().length}" }
                        }
                    }
                ...
                // 保存助手消息并更新会话
                logger.debug { "[$requestId] 保存llm消息到数据库..." }
                ...
                logger.debug { "[$requestId] llm消息保存成功" }
